...
module my_fxn
   ...
   real(kind(0d0)), dimension(1:3,0:3) :: p
   real(kind(0d0)) :: s, sqrts,m_res
   integer         :: i
   contains        
      function dot_vec(p(1,0:3),p(2,0:3)) result(fourvectordot)

      fourvectordot = p(1,0) * p(2,0)
      do i = 1, 3
         fourvectordot = fourvectordot - p(1,i) * p(2,i)
      end do
...
!------------------------------------------!
   subroutine commonpart(t, u, f, tau_0)
      ...
      p(3,0) = sqrt(s)/2
      p(3,1) = 0
      p(3,2) = sqrt(s/4-m**2)*sin_theta
      p(3,3) = sqrt(s/4-m**2)*cos_theta
      p(4,0) = sqrt(s)/2
      p(4,1) = 0
      p(4,2) = -sqrt(s/4-m**2)*sin_theta
      p(4,3) = -sqrt(s/4-m**2)*cos_theta
...
!     I think  subroutine and function are in similar position. x won't be a problem in function either 
      t = (m**2-2*dot_vec(p(1,0:3),p(3,0:3)))
      u = (m**2-2*dot_vec(p(1,0:3),p(4,0:3)))
      f = (m**2-t)*(m**2-u) 
      tau_0 = m**2/A
